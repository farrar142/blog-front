...
export default (props) => {
  const [writeArticle, setWriteArticle] = useWriteArticle();
  const [token, setToken] = useToken();
  const [sysMsg, setMsg] = useSysMsg();
  const curBlog = useBlogChecker(null);
  ...
  const onWriteArticleChangeHandler = () => {
    ...
    setWriteArticle(title, tags, context);
  };
  ...
  const submitHandler = (event) => {
    ...
    Blog.post_article(title, tags, context, token).then((res) => {
      if (res) {
        setMsg("저장되었습니다");
        if (curBlog.id) {
